...
class WCS_WorkPlan
{
    private const TEMPLATE_DIR = 'template/work_plan/';
    ...
    public static function callback_of_export_csv_page(): void
    {
        ...
        $thead_columns = [];
        $tbody_columns = [];
        $table_columns = explode(PHP_EOL, $wcs4_options['work_plan_csv_table_columns']);
    ...
    public static function callback_of_export_html_page(): void
    {
        ...
        $thead_columns = [];
        $tbody_columns = [];
        $table_columns = explode(PHP_EOL, $wcs4_options['work_plan_html_table_columns']);
        foreach ($table_columns as $table_column) {
            [$key, $thead, $tbody] = explode(',', $table_column);
            $thead_columns[trim($key)] = trim($thead);
            $tbody_columns[trim($key)] = trim($tbody);
    ...
    public static function get_items(
        ...
        $table = WCS_DB::get_work_plan_table_name();
        $table_subject = WCS_DB::get_work_plan_subject_table_name();
        $table_teacher = WCS_DB::get_work_plan_teacher_table_name();
    ...
    public static function save_item(bool $force_insert = false): void
    {
        ...
        if (true === $force_insert || current_user_can(WCS4_WORK_PLAN_MANAGE_CAPABILITY)) {
            ...
            if (WCS_DB_WorkPlan_Item::TYPE_PARTIAL === $type) {
                $required['subject_id'] = __('Subject', 'wcs4');
            }
            if (false === $force_insert) {
                $required['type'] = __('Type', 'wcs4');
                if (WCS_DB_WorkPlan_Item::TYPE_CUMULATIVE === $type) {
            ...
            # Validate time logic
            if (false === $force_insert && WCS_DB_WorkPlan_Item::TYPE_CUMULATIVE === $type) {
     ...
     * Renders list layout
     *
     * @param array $work_plans : lessons array as returned by wcs4_get_lessons().
     * @param string $work_plan_key
    ...
    public static function get_html_of_work_plan_list_for_shortcode(
        ...
        $dateWithLessons = [];
        /** @var WCS_DB_WorkPlan_Item $work_plan */
        ...
        $output = '<div class="wcs4-work_plan-list-layout">';
            ...
            if (!empty($dayProgresses)) {
                ...
                foreach ($dayProgresses as $work_plan) {
                    ...
                    if ($work_plan->isTypeCumulative()) {
                        $output .= WCS_Output::process_template($work_plan, $template_periodic);
                    } elseif ($work_plan->isTypePartial()) {
                        $output .= WCS_Output::process_template($work_plan, $template_partial);
                    }
                    $output .= '</li>';
                }
                $output .= '</ul>';
...
add_action('wp_ajax_wcs_add_or_update_work_plan_entry', [WCS_WorkPlan::class, 'save_item']);
add_action('wp_ajax_wcs_add_work_plan_entry', [WCS_WorkPlan::class, 'create_item']);
add_action('wp_ajax_nopriv_wcs_add_work_plan_entry', [WCS_WorkPlan::class, 'create_item']);
add_action('wp_ajax_wcs_delete_work_plan_entry', [WCS_WorkPlan::class, 'delete_item']);
add_action('wp_ajax_wcs_get_work_plan', [WCS_WorkPlan::class, 'get_item']);
add_action('wp_ajax_wcs_get_work_plans_html', [WCS_WorkPlan::class, 'get_ajax_html']);
add_action('wp_ajax_wcs_download_work_plans_csv', [WCS_WorkPlan::class, 'callback_of_export_csv_page']);
add_action('wp_ajax_wcs_download_work_plans_html', [WCS_WorkPlan::class, 'callback_of_export_html_page']);
